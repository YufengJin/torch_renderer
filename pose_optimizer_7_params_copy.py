...
class Model(nn.Module):
    def __init__(self, meshes, renderer, image_ref, calc_grad=False):
        ...
        self.camera_position = nn.Parameter(torch.from_numpy(np.array([0.0400, 0.000,  .7000,  -0.3,  -0.1, .9063, -0.426], dtype=np.float32)).to(meshes.device))
    ...
    def forward(self):
        ...
        loss = self._calc_depth_confidence(depths) + 1*(((quat**2).sum()-1)**2) 
    ...
    def _calc_depth_confidence(self, depths, amp = 1e5, clip_tol = 0.05):
        ...
        inter_mask = torch.tanh(amp*(depths+1.)*(self.image_ref))
        ...
        return (1-(iou+dist_score)/2)*1000
...
plt.subplot(2, 3, 3)
plt.imshow(image_init.detach().squeeze().cpu().numpy()-model.image_ref.cpu().numpy().squeeze())
plt.grid(False)
...
plt.subplot(2, 3, 5)
plt.imshow(model.image_ref.cpu().numpy().squeeze() - image.detach().squeeze().cpu().numpy())
plt.grid(False)
